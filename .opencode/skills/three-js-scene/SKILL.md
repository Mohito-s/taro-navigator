---
name: three-js-scene
description: Use when editing the Three.js cosmic scene in js/space.js (3D moon, starfield, meteors, nebulas, rings, camera, lights) or adding any new 3D element to the TARO background.
---

# Three.js scene

- File: `js/space.js` — an IIFE pinned to `#space` canvas. Three.js r160 is loaded from CDN (`three@0.160.0`, global `THREE`).
...
Serve repo root (e.g. `python -m http.server 8000`), launch `node tools/browser-check/check.cjs` from the repo root, expect 11/11. The moon-position and scrollFade assertions cover the collision bug class.